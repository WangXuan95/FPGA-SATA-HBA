...
本开源库仅提供低性能玩具版本，并用 .dcp 网表对核心代码进行加密。如需正常性能版本的网表、 Verilog 源码、或需要定制其它功能的 SATA 控制器，请联系 629708558@qq.com ，或 QQ : 629708558

# 简介

SATA是硬盘最广泛使用的接口协议。如**图1**是SATA体系结构，其中 SATA host (HBA) 是硬盘读写控制器，在电脑中往往用主板芯片组来实现，而在这里是 FPGA 实现。而 SATA device 就是硬盘（机械硬盘或固态硬盘）。
...
- 虽然没有用硬件实现命令层，但我用硬件实现了一个用 UART 收发 FIS 的示例工程，提供了 UART “FIS透传” 功能：你可以用电脑中的串口软件（比如minicom, putty, hyper-terminal, 串口助手）来发送 FIS ，并能看到 Device 发来的 FIS ，只要我们按命令层规定的 FIS 格式来发送数据，就能直观地实现硬盘的读写。
...
首先我们需要理解 FIS 数据包的结构，如下表。FIS 的长度一定是 4 byte 的倍数。其中 CRC 对于模块调用者是不可见的，**sata_hba_top.sv** 会自动插入、检查并删除。模块调用者仅需要把 FIS-type 和 Payload 字段当成一个整体来发送和接收。在下文的语境中，我们用 **FIS长度** 指代 FIS-type 和 Payload 字段的总长度（最小 2 dwoard, 最大 2049 dword）
...
- rfis_err : 当 rfis_err=1 时，说明 HBA 收到了一个错误的 FIS （CRC错误或长度不合法）。错误的 FIS 仅仅会引起 rfis_err=1 ，而不会影响 rfis_tvalid, rfis_tlast , rfis_tdata 信号，因为本模块内有缓存丢弃机制，错误的 FIS 并不会在 rfis_tdata 上传输。
...
- xfis_err : 当 xfis_done=1 时，xfis_err=1 表示 FIS 传输出错（也即 device 反馈 R_ERR）。
...
我们需要先把它表示成逐个 byte 的格式，注意 dword 是小端序，因此低位 byte 在前，高位 byte 在后：
...
按**命令层**的规定，硬盘会响应两个 FIS ，长度分别为 5 个 dword 和 129 个 dword 。

以**串口助手**软件为例，需要按如**图6**来操作 。关于 identify FIS 包含的格式和信息，请看参考资料[1]，这里不做赘述。
...
以上FIS中， WW=25 代表读命令，WW=35 代表写命令。这些读写命令用 48bit 的 LBA （逻辑扇区地址） 进行寻址， XXXXXX 是 LBA[23:0] ，YYYYYY 是 LBA[47:24] 。ZZ 是读写的扇区数量，一次可以读写一个或多个扇区。
...
然后 HBA 要发送写数据FIS，是 1 dword 的 FIS type (00000046) + 1 个扇区（128 dword) 的数据。比如我们想把前三个字节分别写为 AB CD EF ，其余字节为0，则该 FIS 应为：
